...
import random
import sys
sys.path.insert(1, 'workspace/source/downstream')
...
class CXR_biovil_Trainer():
    ...
    def train(self, epoch):
        ...
        with torch.no_grad():
            ...
            for i, data in test_data_iter:
                ...
                rsm_loss = self.rsm_criterion(projected_findings, projected_impression, data['chexpert_label'])
    ...
    def model_forward(self, data, total_mlm_correct=None, total_mlm_element=None, return_cls_only=False):
        ...
        if USE_SET: # Stage 1: self-questioning
            input_ids, correct_idx, new_txt_labels = self.self_questioning(input_ids, original_ids, attn_masks,segment, txt_labels)
        ...
        if USE_SET: # Stage 2: self-evolution training
            # mlm_loss = self.self_evolution_training(mlm_output, new_txt_labels)
            mlm_loss = self.mlm_criterion(mlm_output.transpose(1, 2), new_txt_labels)
        else:
    ...
    def self_questioning(self,input_ids, original_ids, attn_masks,segment, txt_labels):
        self.model.eval()
        with torch.no_grad(): 
            # lm_predict = self.model(original_ids, attn_masks, segment)['logits'].argmax(dim=-1) * attn_masks
            lm_predict = self.model(input_ids, attn_masks, segment)['logits'].argmax(dim=-1) * attn_masks
        ...
        new_txt_labels = torch.stack([x[1] for x in new_input_ids_and_new_txt_labels])            
        new_txt_labels = (txt_labels*(txt_labels!=-100)) | (new_txt_labels*(new_txt_labels!=-100))
        new_txt_labels = new_txt_labels+((new_txt_labels==0)*-100)
        
        self.model.train()
        
        return new_input_ids, correct_idx, new_txt_labels

    def self_evolution_training(self, mlm_output, txt_labels):
        # with torch.no_grad():
        # lm_original_logits = self.model(original_ids, attn_masks, segment)['logits']
        ...
        # loss = -torch.sum(self.log_softmax(mlm_output[~correct_idx]) * self.softmax(smoothed_logits[~correct_idx])) / mlm_output[~correct_idx].shape[0] 
        # loss = self.mlm_criterion(mlm_output[~correct_idx], smoothed_logits[~correct_idx].argmax(dim=-1))
        return self.mlm_criterion(mlm_output.transpose(1,2), txt_labels)
...
class VLCXR_Trainer():
    def __init__(self, args, train_dataloader, test_dataloader=None, vocab=None, wandb_run=None):
        ...
        if args.weight_load: ## TODO!
            ...
            self.model = CXRFlamingoForPreTraining(args)
        ...
        self.mlm_criterion = nn.CrossEntropyLoss(ignore_index=-100) 
        # TODO: BERT Layer만(x-attn layer는 통과X) forward 해서 얻은 cls representation과 image의 최종아웃풋과 rsm 적용
        self.rsm_criterion = RadiologySelectionLoss(t=0.5)
        # self.itm_criterion = nn.CrossEntropyLoss()
        self.log_softmax = nn.LogSoftmax(dim=-1)
        self.softmax = nn.Softmax(dim=-1)
    ...
    def train(self, epoch):
        ...
        for i, data in train_data_iter:
            with autocast():
                mlm_loss, total_mlm_correct, total_mlm_element, input_ids, original_ids, mlm_output, image_embedding = self.model_forward(data, total_mlm_correct, total_mlm_element)

                mlm_loss = mlm_loss
                
                train_mlm_loss.append(mlm_loss.item())    
                
                # image랑 bert만 통과시킨 cls랑 contrasitive loss
                # rsm_loss = self.rsm_criterion(projected_findings, projected_impression, data['chexpert_label'])
                # train_rsm_loss.append(rsm_loss.item())
            ...
            if (i + 1) % self.args.gradient_accumulation_steps == 0:              
                ...
                if self.step_cnt % self.args.log_freq == 0:
                    # print(f"step {self.step_cnt}, train_loss: {total_loss.item()}, train MLM loss: {mlm_loss.item()}, train RSM loss: {rsm_loss.item()}")
                    print(f"step {self.step_cnt}, train_loss: {total_loss.item()}, train MLM loss: {mlm_loss.item()}")
                    self.print_examples(input_ids, original_ids, mlm_output, n=5)
                    self.wandb_run.log({"training_samples" : self.text_table})
                    log_tanh_gating(self.model, self.wandb_run, self.args)
                self.step_cnt += 1
        ...
        with torch.no_grad():
            ...
            for i, data in test_data_iter:
                ...
                mlm_loss, total_mlm_valid_correct, total_mlm_valid_element, input_ids, original_ids, mlm_output, image_embedding = self.model_forward(data, total_mlm_valid_correct, total_mlm_valid_element)
            ...
            print("avg loss in testset", np.mean(eval_losses))
            # print(f"avg MLM loss: {np.mean(eval_mlm_loss)}, avg RSM loss: {np.mean(eval_rsm_loss)}")
            print(f"avg MLM loss: {np.mean(eval_mlm_loss)}")
            self.print_examples(input_ids, original_ids, mlm_output, n=5)
    ...
    def model_forward(self, data, total_mlm_correct=None, total_mlm_element=None, return_cls_only=False):
        ...
        input_ids, txt_labels, attn_masks, segment, original_ids = data['findings_tensors']
        ...
        if self.args.use_prev_img:
            prev_image = data['prev_image'].to(self.device)
        
        images = (image, prev_image)
        # images = image
        ...
        # if USE_SET: # Stage 1: self-questioning
        #     input_ids, correct_idx, txt_labels = self.self_questioning(input_ids, original_ids, attn_masks,segment, txt_labels)

        mlm_output, image_embedding = self.model(report=(input_ids, segment, attn_masks), images=images)
        ...
        return mlm_loss, total_mlm_correct, total_mlm_element, input_ids, original_ids, mlm_output, image_embedding
    ...
    def save(self, epoch, file_path):
        ...
        if torch.cuda.device_count() > 1:
            torch.save(self.model.module.state_dict(), os.path.join(save_path_per_ep,'pytorch_model.bin'))
            print(f'Multi_EP: {epoch} Model saved on {save_path_per_ep}')
...
def log_tanh_gating(model, wandb_run, args):
    ...
    for i, layer in enumerate(m.encoder.encoder_layers):
    ...
    wandb_run.log(att_gate_gain)
    wandb_run.log(ffw_gate_gain)